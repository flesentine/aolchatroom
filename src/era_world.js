...
const FUTURE_CONSOLES = /\b(?:ps[2-5]|playstation\s*[2-5]|dreamcast|gamecube|xbox(?:\s*(?:360|one|series\s*[sx]))?|wii(?:\s*u)?|nintendo\s+switch|switch\s+lite|game\s+boy\s+advance|gba|nintendo\s+ds|(?:nintendo\s+)?3ds|psp|playstation\s+portable|ps\s*vita)\b/i;
...
export function eraWorldViolation(text, dateKey = "1996-01-01") {
  ...
  if (FUTURE_CONSOLES.test(value)) return "future-console";
