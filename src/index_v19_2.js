import baseWorker, { ChatRoom as ProviderFixChatRoom } from "./index_v19_1.js";
...
export class ChatRoom extends ProviderFixChatRoom {
  constructor(ctx, env) {
    super(ctx, env);
    ...
    try {
      const data = await this.env.AI.run(model, {
        messages: [
          {
            role: "system",
            content: `It is ${simulatedDateTimeLabel()}. The world is 1996. You are simulating fictional adult AOL chat users. Never act like an assistant. Return only valid JSON.`
          },
  ...
  async callGroq(prompt, maxTokens = 340, maxMessages = 5, defaultTarget = "room") {
    ...
    for (const provider of providers) {
      ...
      if (!result?.ok) {
        // A successful HTTP/Workers-AI invocation with no usable model text is a model-output
        // miss, not an outage. Keep the provider ready and try the next one immediately.
    ...
    // Do not replace a known-good provider status with a scary transient message just
    // because one scene plan was rejected by our realism filters.
    const stillReady = this.configuredProviders().some((provider) => this.providerReady(provider, Date.now()));
    if (stillReady && this.lastSuccessfulProvider) {
      this.setAiStatus(`AI active · ${LABELS[this.lastSuccessfulProvider] || this.lastSuccessfulProvider}`);
    } else if (stillReady) {
      this.setAiStatus("AI ready · retrying scene");
    } else {
